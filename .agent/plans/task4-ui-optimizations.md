...
> **已被 Task 5 调整（2026-09）**：需求改为「显示完整」，`MAX_CROP_FACTOR` 由 `1.4` 回退到 `1.0`
> （不再裁切、画面不足的一边留黑边）。本节保留当时的数据与推导，方案与量化见 `.agent/plans/task5-preview-fit.md`。
...
1. 预览：采用**方案 A** —— 宽度铺满 + 垂直居中裁切，裁切上限 `MAX_CROP_FACTOR = 1.4`（≈ 最多裁帧高 30%）。
   （2026-09 Task 5 已按「完整显示」需求回退为 `1.0`，见第 10.1 节顶部说明。）
